...
public static class TileRenderer
{
    public static BaseShape Tessellate(this MapFeatureData feature, ref BoundingBox boundingBox, ref PriorityQueue<BaseShape, int> shapes)
    {
        ...
        if (feature.Properties.Any(p => p.Key == EPropertyKeys.Highway && MapFeature.HighwayTypes.Any(v => String.Equals(p.Key.ToString(), v, StringComparison.OrdinalIgnoreCase))))
        {
        ...
        else if (feature.Properties.Any(p => p.Key == EPropertyKeys.Water) && feature.Type != GeometryType.Point)
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == EPropertyKeys.Railway))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == EPropertyKeys.Natural && featureType == GeometryType.Polygon))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == EPropertyKeys.Boundary && p.Value == EPropertyValues.Forest))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Properties.Any(p => p.Key == EPropertyKeys.Landuse && (p.Value == EPropertyValues.Forest || p.Value == EPropertyValues.Orchard)))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Forest);
            baseShape = geoFeature;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p=> p.Key == EPropertyKeys.Landuse && (p.Value == EPropertyValues.Urban)))
        {
            var coordinates = feature.Coordinates;
            ...
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                     ...
                     => p.Key == EPropertyKeys.Landuse && (p.Value == EPropertyValues.Farmland)))
        {
            var coordinates = feature.Coordinates;
            ...
            shapes.Enqueue(geoFeature, geoFeature.ZIndex);
                 ...
                 feature.Properties.Any(p => p.Key == EPropertyKeys.Landuse && (p.Value == EPropertyValues.Waterworks)))
        {
            var coordinates = feature.Coordinates;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == EPropertyKeys.Building))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == EPropertyKeys.Leisure))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
        ...
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == EPropertyKeys.Amenity))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
